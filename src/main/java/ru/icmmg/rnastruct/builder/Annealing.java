...
public class Annealing {
  ...
  public Annealing(
    ...
    requiredSeqNumber = params.getSeqNumber() != null ? params.getSeqNumber() : seqLengths.size();
  ...
  public TreeItem start() throws IOException {
    ...
    while (changeCounter < params.getNumberIterationToStop()) {
      iterationCounter++;
      logString("Current iteration: " + String.valueOf(iterationCounter));
      TreeItem probNext;
      ...
      logString("next pattern: \n" + PatternUtils.writeTree(probNext));
      logString("prob next func value = " + probNextFunc);
      logString("current func value = " + currentValue);
    ...
    logString(PatternUtils.writeTree(currentPat));
    return currentPat;
  }
  ...
  double function(TreeItem pattern) throws IOException {
    ...
    while (scanner.hasNextLine()) {
      ...
      if (xM.find() && xM.group(1).equals("0")
          && lenM.find()
              && Integer.valueOf(lenM.group(1)).equals(seqLengths.get(nameM.group(1)))) {
        logString("+");
